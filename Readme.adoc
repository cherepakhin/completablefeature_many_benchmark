...
И все таки насколько ценен CompletableFeature? Может быть лучшие результаты даст кеш или оптимизация сервисов, которые позади REST? По своему опыту, программирование с CompletableFeature, мягко говоря, очень специфично, если не держать его под контролем. Возвращая CompletableFeature в методе, вы принуждаете клиента его обрабатывать.
